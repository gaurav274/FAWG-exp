...
func MakeRequest(url string, values map[string]interface{}, ch chan<- string,
	deadline_ms float64) {
	...
	// data_int, _ := strconv.Atoi(values["data"])
	// type Value map[string]interface{}
	// new_values := Value{
	// 	"data":            data_int,
	// 	"absolute_slo_ms": current_time_str,
	// }
...
func main() {
	...
	values := map[string]interface{}{"data": data_int}
	...
	for i := 0; i < len(arrival_curve); i++ {
		...
		go MakeRequest("http://127.0.0.1:8000/noop", values, ch, deadline_ms)
	}
	for i := 0; i < len(arrival_curve); i++ {
		<-ch
	}
