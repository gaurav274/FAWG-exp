...
def main(xls_file, start_cmd, end_cmd):
    ...
    for index, row in df.iterrows():
        ...
        # latency calculation
        latency_list = []
        for i in range(df.loc[index, "# requests"]):
            temp = {}
            temp["start"] = time.time()
            ray.get(ppu.remote(data=1))
            temp["end"] = time.time()
            latency_list.append(temp)
    ...
    with pd.ExcelWriter(xls_file, mode="a") as writer:
        new_df_clean.to_excel(writer, sheet_name="Closed Loop Results")
        new_df_raw.to_excel(
            writer, sheet_name="Closed Loop Results(Raw)"
        )
